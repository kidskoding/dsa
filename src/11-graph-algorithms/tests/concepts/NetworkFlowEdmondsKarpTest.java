...
class NetworkFlowEdmondsKarpTest {
	...
	void diamondMaxFlow() {
		...
		assertEquals(4.0, new NetworkFlowEdmondsKarp().maxFlow(g, 0, 3));
	}
	...
	void singleEdgeBottleneck() {
		...
		assertEquals(2.0, new NetworkFlowEdmondsKarp().maxFlow(g, 0, 2));
	}

	@Test
	void noPathYieldsZeroFlow() {
		WeightedGraph g = new WeightedGraph(2);
		assertEquals(0.0, new NetworkFlowEdmondsKarp().maxFlow(g, 0, 1));
	}
}
